...
full dataset with $k=7$ on a single notebook computer took 1 minutes. Scipy
SVD calculation on the same dataset is much faster, however, map/reduce
environment incrues certain costs during its start-up and preparation
phases for its jobs, as well as outputting, reading to and from disks due
to the record-by-record processing nature of Hadoop can introduce
latencies. However, exactly for the same reasons a map/reduce algorithm can
scale horizontally, processing records in the billions proportional to the
number of nodes in a cluster. All code relevant for this paper can be found
here
