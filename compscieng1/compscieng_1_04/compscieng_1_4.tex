...
Isimiz bitti mi? Hayir. Ikinci türevin sýfýra eþit oldugu iki cozum daha
lazým, iki homojen cozum yani, bunlardan biri $C$ digeri $Dx$. Cunku
elimizde tatmin edilmesi gereken iki tane sýnýr sartý var. 
...
Nokta altinda sýkýsma, ustunde ise esneme olur. 

$u(x)$ tabii ki hep pozitiftir, yani cubugun tum noktalari asagi iner. Ama
bazi noktalar uzerinde sýkýsma, pozitif egim, digerleri uzerinde esneme
(negatif egim) vardir. 
...
Gri olarak nitelenen yer $a$'nin altinda olan yer, ve ol bolum bir sýkýþma
...
$-A$ ve $A$ entegral sýnýrlarý sýfýrý ortalayacak sekilde secilmis iki
...
Simdi sýnýr sartlarini kullanarak $u(x)$ icinde yerine koyalim:
...
\includegraphics[height=4.5cm]{4_16.png}

(28)'deki geriye dogru farklar matrisi $\Delta_{-}$ tersinin (29)'daki
toplamlar matrisi oldugunu kontrol edin. Dikkat: $\Delta_0 = (\Delta_{+} +
\Delta_{-}) / 2$ ifadesinin tersi olmayabilir! $\Delta_0 u = 0$ denklemini
$n=3$ ve $n=5$ icin cozun.
...
Bir merkezi farksal yaklasiksallama (difference approximation) kur, bunu
yaparken $K/h^2$ ve $\Delta_0 / 2h$ kullan, tum bunlari 

\[ -u'' + u' = 0. \]

cozmek icin kullan, ki $u(0) = 0$ ve $u(1) = 0$. Ayri olarak $du/dx$ icin
one dogru farksal (forward difference) $\Delta_{+}U/h$ kullan. Dikkat
edelim $\Delta_0 = (\Delta_{+} + \Delta_{-}) / 2$. Ortalanmis $u$ ve
ortalanmamis $U$'yu cozelim, ki $h = 1/5$ olsun. Gercek $u(x)$ $u=x$ ozel
cozumudur ve bu cozume $A+Be^x$ eklenir. Hangi $A$ ve $B$ sinir sartlarini
tatmin eder? $u$ ve $U$ ne kadar $u(x)$'e yakindir? 
