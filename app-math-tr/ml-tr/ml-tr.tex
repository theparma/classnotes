...
$\mathbf{x}$'in bütün elemanlarý tek bir özelliði ölçen büyüklükler olarak kabul
edelim, ve tersi belirtilmezse birbirinden baðýmsýz olan ölcümler (iid)
olarak görelim. 
...
Ýlk önce (\ref{eq:reg1})'i matris formuna çeviriyoruz, ki doðrusal cebir
...
(\ref{eq:reg2})'da gösterilen toplama dikkat edersek, (\ref{eq:reg3})
 formülünden kaybolduðunu göreceðiz. Toplamýn yerine ilk matriste ek boyutlar
 geldi. Doðrusal cebirin biraz daha eklenmesi, tüm formülü daha da temiz hâle
 getirecektir. 
...
Yeni risk hesabý, N veri deðeri ve D boyutu için
kullanýlabilir. $\theta$ hesabýný ise gene aynen \ref{eq:reg7}
denklemi üzerinden gerçekleþtirebiliriz. Bu denklemden gelen sonuç, N
boyutlu bir hiper düzlemin hâm veriye en uygun (fit) hâli olacaktýr. 
...
Ve hâla \ref{eq:reg7} denklemini regresyon için kullanabiliriz. 

Regresyonun Bazý Sorunlarý

Regresyon metodunun en temel eksiklerinden birisi, hâm veriyi
modellemek için elimizde saðlam matematiksel temelin olmamasýdýr. Daha
önce gördüðümüz gibi kayýp fonksiyonu rasgele seçtik. Kayýp
fonksiyonu, uzaklýðýn karesi olduðu gibi küpü de rahatlýkla
olabilirdi. Hangisi daha iyi bir seçimdir? Elimizdeki yöntem bu
cevaplarý vermemektedir. Bazý ek yaklaþýmlar bu sorunu eðitim verisini,
``eðitim bölümü'' ve ``eðitimin performansýný kontrol bölümü'' olarak
ikiye bölerek düzeltmeye uðraþtýlar. Bu þekilde ele geçen modellerden
hangisinin daha iyi olduðu ölçümsel þekilde bulunmaya uðraþýldý. Fakat
ideal olarak, her problem için tek yaklaþýmý takip etmek bizim için
daha uygundur. 

Ayrýca, regresyonu tekniðini sýnýflandýrma (classification) amaçlý kullanmak
bazý problemleri beraberinde getirmektedir. Meselâ alttaki sekildeki 
en ideal sýnýf ayraç çizgisinin, kesikli olan çizgi olduðu bellidir.  
...
Fakat, bu verideki en uzak sað ve sol noktalar, regresyon tarafýndan
daha fazla cezalandýrýlmakta, bu yüzden uzaklýklarý sebebiyle
deðerleri büyük olduðu için regresyon çizgisi onlara daha fazla
yakýnlaþmaya çabalamaktadýr. Bunun sonucu da düz çizgi olarak
görünen ayraç olmaktadýr. Bu soruna bir mühendislik çözümü bir
ezici/düzeltici  (squashing) fonksiyonu kullanmaktýr, böylece uzaktaki
noktalarýn cezalandýrýlmasýnýn tersi gerçekleþtirilmeye uðraþýlýr. Popüler 
düzeltici fonksiyonlardan biri $g(z) = (1 + exp(-z))^{-1}$ ve
perceptron inþa etmek için kullanýlan $sign$ fonksiyonudur. 
...
parametrelerini bile bir rasgele deðiþken olarak gösterebilirsiniz. Böylece
diðer rasgele deðiþkenler ile bu tür deðiþkenler arasýnda baðýmlýlýk iliþkisi
kurulabilir. Meselâ, alttaki sekildeki noktalarý sýnýflandýrma örneðinden
baþlayalým.
...
Büyük Olabilirlik (Maximum Likelihood) gibi bir metoda girdi olarak verebiliriz,
...
Sýnýflandýrma amacýmýza eriþebilmek için \ref{eq:graphjoint1} formülünü, 
$p(x,y|\alpha,\mu,\Sigma)$ sorusunu cevaplandýrabilen bir hâle
getirmeliyiz. Olasýlýk kuramýndan bilinen bir deðiþim ile pür cebirsel olarak 
þöyle devam edebiliriz.
...
Þimdi \ref{eq:graphjoint2}'deki bölüneni, grafik modelden gelen ortak daðýlým
\ref{eq:graphjoint1} ile deðiþtireceðiz.
...
Olabilirlik yöntemini kullanacaðýz. Bu metoda göre \ref{eq:graphjoint3} formülünün
her N veri noktasý ile hesabýndan sonra birbiri ile çarparýz. O zaman, bilinmeyen
parametrelerin en iyi hesapsal tahmini (estimation), bu süper ortak daðýlýmýn
en yüksek olduðu noktada bulunabilir.
...
Olabilirliðin en yüksek olduðu nokta, olabilirlik formülünün türevinin sýfýr
olduðu noktadadýr. 

Ýleride yapýlacak türev alma iþlemini rahatlatmak için, \ref{eq:likelihood1}
içindeki terimlerin log'unu alýrýz. Bu iþlem, çarpým iþaretini toplam iþaretine
dönüþtürecektir. Bunu yapmamýz en yüksek noktayý bulma açýsýndan bir fark
yaratmaz, çünkü \ref{eq:likelihood1} formülünün deðiþim hýzý,
toplamlý olan formülünkiyle aynýdýr.
...
birincisi hariç \ref{eq:likelihood3} içindeki bütün terimler yokolur. Sonuç
olarak þu olur:
...
yöntemi takip edebiliriz. \ref{eq:likelihood3}'in türevini bilinmeyen rasgelen
deðiþkene göre alýrýz, sýfýra eþitleriz ve çözeririz. Bu iþlem yer darlýðý
sebebiyle burada gösterilmeyecektir. Bu iþlemlerin sonunda formül
\ref{eq:p_x_y}'deki tüm bilinmeyenleri bulabiliriz. \ref{eq:p_x_y}'i daha
önce bulunmuþ olan $p(y)$ ile kullanýnca, \ref{eq:graphjoint3}'i nihayet
sýnýflama için kullanmamýz mümkün olacaktýr. Yapay Öðrenim literatüründe $p(y)$,
ilk tahmin (prior guess) olarak da bilinir. 
