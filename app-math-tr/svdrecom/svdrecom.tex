...
gerceklestirilir, daha fazla detay icin {\em Lineer Cebir 33}'e bakiniz)
...
Dogal olarak \verb!1e-17! gibi bir sayi sifira cok yakin, yani sifir kabul
...
yapalim, ve sonuc icinde bir tane bile True olup olmadigini kontrol edelim,
...
Bir diger yontem [1] yeni Bob verisi $y$'yi alip 

$$ z = VV^Ty $$

olarak $z$'ye cevirmek. Bu durumda aslinda cebirsel olarak hicbir sey
yapmamis oluyoruz,

$$ z = VV^Ty = Iy = y$$

ve iteratif sayisal cogu algoritmanin temelini de bu olusturuyor. Kavramsal
olarak $y$'yi alip $V$ uzayina ``yansitiyoruz''. Daha kavramsal olarak
kullanici secimlerini temsil eden veri icin $V$ bir ``kordinat sistemi''
olusturmustur (SVD'nin dogal sonucu olarak) ve her veri noktasi bu kordinat
sistemi, bu bazin vektorlerinin bir kombinasyonu olarak temsil edilebilir
durumdadir (SVD icin kullanilan veriden bahsediyoruz). Bu durumda yeni
veriyi oraya yansitmak dogal bir islemdir. Tabii yansitip sonra geri
geliyoruz, yani baslangictaki boyutlara / hale donuyoruz, bu olurken ayni
zamanda Bob verisinin bos noktalari en makul tahminlerle ``doldurulmus''
oluyor. 
...
Sonuca gore Bob buyuk ihtimalle S5'i sevecektir, not tahminleri arasinda en
yuksek puan orada tahmin edilmis, ki bu daha onceki Ben ve Fred benzerlik
tahminleri ile uyumlu. 

Not: Dongude $z$'nin hep ayni satir olmasi kafa karisikligi yaratmasin, bu
cok ufak bir veri seti, daha buyuk veri setlerdinde bu degisim
gorulecektir. 

Iteratif islem taklit kod olarak,

\begin{algorithm}[h]
\begin{pseudocode}
\codename $\code{imputed\_svd}$\\
\codeline $\code{while }$ $z$'deki degisim azalincaya kadar (convergence)\\
\codeline \>  $z = VV^Ty$  \\
\codeline \>  $y$'nin ilk halindeki bilinen noktalari alip $z$'ye kopyala  \\
\end{pseudocode}
\end{algorithm}

En son projemizde ustteki islemin en iyi sonuclar verdigini gozlemledik. 
...
[5] url{https://anandoka.wordpress.com/tag/imputed-svd}
