...
Matematik'in en önemli yararý, mantýksal sonuçlara varabilmek icin
...
bir ortam saðlamasi da önemli bir avantaj.
...
hakkýnda bilgi ve vizyon gerektirir. Taslak bir modelle baþlayabilirsiniz,
ve testleriniz sonucunda modeli safileþtirmek (rafine etmek)
mümkündür. Mesela yukarýdaki örnek için baþlangýçta sürtünme kuvvetini
hesaba katmayabilirsiniz, fakat ileride daha net sonuçlar için sürtünmeyi
modele eklemeniz gerekebilir.
...
Fakat, insanlarýn sadece doðal sebeplerden öldüðünü varsayarsak, ölüm
...
Burada $p$ ve $p^2$ ifadelerinin katsayilari, sabitleri degisik, cunku bir
sabit normal dogum olumu, digeri ise kaynaklara olan yaristaki olumler icin
secildi. 

Simdi biraz notasyon degisikligi, Cebirsel islemleri kolaylastirmak icin
sabitlerde biraz oynama yapacagiz. $a = k$, ve $b = k/K$ diyelim. 
...
Esitligin sol tarafinda entegralin icindeki ifadede hem bolum, hem boleni
$K$ ile carparsak

\[ = \frac{K}{p(K-p)} \]

Bunun uzerinde kismi kesirler yontemini (partial fractions method)
kullanabiliriz, ozellikle bolumdeki carpanlarin toplaminin bolumde oldugu
durumlarda, ki bizim ornegimizde bu $p + K - p = K$, bu ifadeyi ayirmak cok
kolay, kismi kesirlerin bolumu hemen 1 olabilir
...
\[ \int \frac{dp}{p} + \int \frac{dp}{K-p} = \int k \ dt \]
...
Peki $A$ nedir? Eger (*) ifadesine $t=0$ verirsek
...
Elde ettigimiz $p(t)$ fonksiyonunu grafikledigimiz zaman cikti suna
benzeyecektir. 

\includegraphics[height=6cm]{hill.png}

Elde edilen sekilde bir 'S' goruntusunde, bu fonksiyona ``S egrisi'' adi
verildigi de oluyor. Bir diger isim ``Hill fonksiyonu''. 

Veri Uydurmak 

Hill fonksiyonunu sayisal yontemler kullanarak tarihi veriye uydurarak,
sabitlerini hesaplatmak mumkundur. Scipy paketindeki
\verb!scipy.optimize.leastsq! fonksiyonu bu isi yapabilir. Uydurmanin
islemesi icin once bir \verb!f! tanimlanir, daha sonra bu fonksiyonun
gercek veri ile arasindaki hatayi tanimlayan bir fonksiyon verilir. Bu iki
fonksiyon uzerinden veri uydurmasi yapilacaktir. 

\lstinputlisting[language=Python]{us.py}

Program isletildikten sonra grafik basilacak. Grafikte kirmizi noktalar
gercek veri, yesil noktalar ise uyumu yapilmis fonksiyonun gelecek yillar
icin gosterdigi tahmindir. 
...
gördük. Türevsel denklem kurarken düþünülmesi gereken, $dp/dt$ görünce 'oran'
düþünmek, yani akla 'deðiþim' getirmek. Tabii daha türevleri daha detaylý
anlayabilmek için (ispatý ile birlikte) limitlerin kuramý yararlý olur,
fakat unutmayalým ki limitlerin analiz'in ispatý için yetiþinceye kadar
yüzyýllar geçmiþti! Analiz'in tam ispatý daha gelmeden, mühendisler ve
bilim adamlarý türevleri ve tümlevleri kullanýyorlardý.

Analiz, dinamik olan sistemler için kullanýlýr, yani deðiþmekte olan
sistemler için gereklidirler. Analiz'in tarihinin fizik ile yakýn alakasý
bundandýr.

Kaynaklar

http://www.math.northwestern.edu/~mlerma/courses/math214-2-03f/notes/c2-logist.pdf

Cliburn Chan, 2010, {\em Scientific Scripting with Python for Computational Immunology}
