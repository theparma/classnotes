...
Aslinda akýþ cizgisel entegrallerin degisik bir seklidir. Diyelim ki $C$
...
Bu kavrami daha once hesapladigimiz ``yapilan iþ'' karsilastirmak
gerekirse

\[ Ýþ = \int_C \vec{F} \cdot d\vec{r} = \int_C \vec{F} \cdot \vec{T} \ ds \]

Ýþi hesaplarken her noktada $\vec{F}$'in egri $C$'nin tegetine yansimasini
hesapliyorum. 

\includegraphics[height=2cm]{23_2.png}

Yani egri uzerinde hareket ederken ``$\vec{F}$ ile ne kadar birlikte, ne
kadar ona ters sekilde'' hareket edildiginin hesabini yapilan iþ veriyor. 

Kiyasla akýþ hesabi, egri uzerinde hareket ederken $\vec{F}$'in egrinin
``ne kadar icinden gectiginin'' hesabi. Ruzgarli bir havada bir yolda
yuruyorum, akýþ ruzgarin bana ne kadar yandan carptigini gosteriyor. Akis
...
ise, akýþ $\vec{F}$'in normal bilesenlerinin toplamidir. Bu ufak fark
...
anlatimi rahatlatiyor ($\vec{F}$ baska bir sey de olabilir). Akýþ
baglaminda $\vec{F}$'i bir hiz alani (velocity field) olarak gormek
benzer bir rahatlik sagliyor. 

Bir siviyi dusunursek, akýþ birim zamanda $C$'nin icinden ne kadar sivinin
...
Tum bu hesaplari her parca icin toplarsak akýþý elde ederiz. 
...
Bu buyukluk beklenebilecegi uzere pozitif, cunku akýþ cemberden disari
dogru. 
...
bileseni olamaz, yani sifir olur. Burada akýþ olmadigini ciplak gozle bile
gorebiliyoruz. $\vec{F} \cdot \hat{n} = 0$, o zaman akýþ = 0. 
...
$Mdx +
Ndy$ hesabini yaptigimiz gibi. Akýþ hesabinda benzer bir yaklasim var. 
...
entegralinin yerine bir cift entegral gecirmek. Cevap evet, akýþ hesabi
icin de isleyen Green Teorisi'nin bir versiyonu var. 

Akýþ Icin Green Teorisi
...
Akýþ hesabi, o zaman, $C$ icinden gecerek $R$'den disari cikan akýþ
...
ve bunlarin toplami tum akýþi verecektir. 
...
1) $div$ akýþýn ne kadar ``yayildigini'', ``genisledigini'' olcer
diyebiliriz. Mesela cemberli ornekte her sey disari cikiyor, bir genisleme
var, $div$ pozitif. Eger akýþ tersine olsaydi (her sey iceri dogru) o zaman
$div$ negatif olurdu. 
